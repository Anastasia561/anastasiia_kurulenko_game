package ua.javarush.processor;

import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.junit.jupiter.MockitoExtension;

import java.lang.reflect.Field;

import static org.junit.jupiter.api.Assertions.*;

@ExtendWith(MockitoExtension.class)
public class GameProcessorTest {
    private static final GameProcessor gameProcessor = new GameProcessor();

    @BeforeAll
    public static void init() {
        gameProcessor.createGame("/testGame.json");
    }

    @Test
    public void createGameTestShouldThrowException() {
        assertThrows(IllegalArgumentException.class, () -> {
            gameProcessor.createGame("text");
        });
    }

    @Test
    public void generateQuestionTextTest() throws IllegalAccessException, NoSuchFieldException {
        Field field = gameProcessor.getClass().getDeclaredField("questionId");
        field.setAccessible(true);
        field.set(gameProcessor, 0);

        String expected = "questionText0";
        String actual = gameProcessor.generateQuestionText();
        assertEquals(expected, actual);
    }

    @Test
    public void generateFirstAnswerTextTest() throws IllegalAccessException, NoSuchFieldException {
        Field field1 = gameProcessor.getClass().getDeclaredField("questionId");
        field1.setAccessible(true);
        field1.set(gameProcessor, 0);

        String expected = "answerText0";
        String actual = gameProcessor.generateFirstAnswerText();
        assertEquals(expected, actual);
    }

    @Test
    public void generateSecondAnswerTextTest() throws NoSuchFieldException, IllegalAccessException {
        Field field1 = gameProcessor.getClass().getDeclaredField("questionId");
        field1.setAccessible(true);
        field1.set(gameProcessor, 0);

        String expected = "answerText1";
        String actual = gameProcessor.generateSecondAnswerText();
        assertEquals(expected, actual);
    }

    @Test
    public void generateAnswerMessageTestShouldReturnEmptyString() {
        String expected = "";
        String actual = gameProcessor.generateAnswerMessage(null);
        assertEquals(expected, actual);
    }

    @Test
    public void generateAnswerMessageTestShouldReturnNonemptyString() {
        String expected = "answerMessage0";
        String actual = gameProcessor.generateAnswerMessage("0");
        assertEquals(expected, actual);
    }

    @Test
    public void generateNextQuestionIdTestWhenAnswerIndexIsNull() {
        boolean actual = gameProcessor.generateNextQuestionId(null);
        assertTrue(actual);
    }

    @Test
    public void generateNextQuestionIdTestWhenAnswerIndexIsNotFailIndex() {
        boolean actual = gameProcessor.generateNextQuestionId("0");
        assertTrue(actual);
    }

    @Test
    public void generateUserNameTestIfProvidedIsNull() throws NoSuchFieldException, IllegalAccessException {
        Field field = gameProcessor.getClass().getDeclaredField("username");
        field.setAccessible(true);
        field.set(gameProcessor, "User");

        String expected = "User";
        String actual = gameProcessor.generateUsername(null);
        assertEquals(expected, actual);
    }

    @Test
    public void generateUserNameTestIfProvidedIsNotNull() {
        String expected = "user3";
        String actual = gameProcessor.generateUsername("user3");
        assertEquals(expected, actual);
    }

    @Test
    public void generateProgressTestForQuestionId0() throws NoSuchFieldException, IllegalAccessException {
        Field field = gameProcessor.getClass().getDeclaredField("questionId");
        field.setAccessible(true);
        field.set(gameProcessor, 0);

        String expected = "0";
        String actual = gameProcessor.generateProgress();
        assertEquals(expected, actual);
    }

    @Test
    public void generateProgressTestForQuestionId1() throws NoSuchFieldException, IllegalAccessException {
        Field field = gameProcessor.getClass().getDeclaredField("questionId");
        field.setAccessible(true);
        field.set(gameProcessor, 1);

        String expected = "33";
        String actual = gameProcessor.generateProgress();
        assertEquals(expected, actual);
    }

    @Test
    public void generateProgressTestForQuestionId2() throws NoSuchFieldException, IllegalAccessException {
        Field field = gameProcessor.getClass().getDeclaredField("questionId");
        field.setAccessible(true);
        field.set(gameProcessor, 2);

        String expected = "66";
        String actual = gameProcessor.generateProgress();
        assertEquals(expected, actual);
    }

    @Test
    public void generateEndGameMessageTestForVictory() throws NoSuchFieldException, IllegalAccessException {
        Field field1 = gameProcessor.getClass().getDeclaredField("questionId");
        field1.setAccessible(true);
        field1.set(gameProcessor, 2);

        Field field2 = gameProcessor.getClass().getDeclaredField("answerId");
        field2.setAccessible(true);
        field2.set(gameProcessor, 0);

        String expected = "You won!";
        String actual = gameProcessor.generateEndGameMessage();
        assertEquals(expected, actual);
    }

    @Test
    public void generateEndGameMessageTestForFailureIfAnswerIdIsNegative() throws NoSuchFieldException, IllegalAccessException {
        Field field1 = gameProcessor.getClass().getDeclaredField("answerId");
        field1.setAccessible(true);
        field1.set(gameProcessor, -1);

        String expected = "Failure";
        String actual = gameProcessor.generateEndGameMessage();
        assertEquals(expected, actual);
    }

    @Test
    public void generateEndGameMessageTestForFailureIfQuestionIdIsNotMax() throws NoSuchFieldException, IllegalAccessException {
        Field field1 = gameProcessor.getClass().getDeclaredField("questionId");
        field1.setAccessible(true);
        field1.set(gameProcessor, 1);

        String expected = "Failure";
        String actual = gameProcessor.generateEndGameMessage();
        assertEquals(expected, actual);
    }

    @Test
    public void generateResultImgPathTestForVictory() throws IllegalAccessException, NoSuchFieldException {
        Field field1 = gameProcessor.getClass().getDeclaredField("questionId");
        field1.setAccessible(true);
        field1.set(gameProcessor, 2);

        Field field2 = gameProcessor.getClass().getDeclaredField("answerId");
        field2.setAccessible(true);
        field2.set(gameProcessor, 0);

        String expected = "data:image/jpeg;base64,";
        String actual = gameProcessor.generateResultImgPath();
        assertEquals(expected, actual);
    }

    @Test
    public void generateResultImgPathTestForFailureIfAnswerIdIsNegative() throws NoSuchFieldException, IllegalAccessException {
        Field field1 = gameProcessor.getClass().getDeclaredField("answerId");
        field1.setAccessible(true);
        field1.set(gameProcessor, -1);

        String expected = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTcy7WoHI7Z1TMlAfPo0AY2BxwfaPFDL8Rk3RTIH_XRXyuoMJivUhfxFDWLG3Jyx3FA6j8&usqp=CAU";
        String actual = gameProcessor.generateResultImgPath();
        assertEquals(expected, actual);
    }

    @Test
    public void generateResultImgPathTestForFailureIfQuestionIdIsNotMax() throws NoSuchFieldException, IllegalAccessException {
        Field field1 = gameProcessor.getClass().getDeclaredField("questionId");
        field1.setAccessible(true);
        field1.set(gameProcessor, 1);

        String expected = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTcy7WoHI7Z1TMlAfPo0AY2BxwfaPFDL8Rk3RTIH_XRXyuoMJivUhfxFDWLG3Jyx3FA6j8&usqp=CAU";
        String actual = gameProcessor.generateResultImgPath();
        assertEquals(expected, actual);
    }
}
